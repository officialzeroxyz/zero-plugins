...
- **The runner** — a `zero` CLI the extension provisions for this session at a stable absolute path.
  This is your primary tool: it runs the whole loop, handles 402 payment (including cross-chain),
  streams binary output, and enforces spend caps.
...
> **Convention:** examples below write `zero` for brevity. The SessionStart hook announces the
> runner's absolute path (default `~/.zero/runtime/bin/zero`) — invoke it by that path. Unlike some
> hosts, **Gemini CLI does not export the runner as a `$ZERO_RUNNER` environment variable to your
> shell**, so do not rely on `$ZERO_RUNNER` expanding; use the literal absolute path the hook
> reported. If the SessionStart hook reported the runner as unavailable, in a persistent environment
> tell the user Zero isn't available here rather than improvising; in an ephemeral sandbox, invoke
> the published CLI directly with `npx` instead (see **Ephemeral / sandbox** below). Never create a
> wallet either way.

## The runner

The runner is the published `@zeroxyz/cli`, installed once per session into a shared,
extension-owned home and announced by its absolute path (default `~/.zero/runtime/bin/zero`). You do
**not** install, update, or configure it — the SessionStart hook handles provisioning, and the
runner needs no wallet setup. Identity comes from a session (below); signing is managed server-side.
...
1. **Get the runner.** Use the runner path the SessionStart hook announced if it provisioned one;
   otherwise invoke the CLI with `npx -y @zeroxyz/cli@latest` (node/npx is present in most sandboxes)
   — substitute that wherever the loop writes `zero`.
