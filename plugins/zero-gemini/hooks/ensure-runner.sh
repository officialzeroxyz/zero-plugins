#!/usr/bin/env bash
#
# Zero extension (Gemini CLI) — SessionStart hook.
#
# Provisions a Zero "runner" so the agent can call paid capabilities directly in its
# own environment and tells the agent — via the SessionStart `additionalContext`
# payload — the runner's absolute path and how to authenticate.
#
# Gemini note: unlike Claude Code, Gemini CLI has no session env-file a hook can write
# to, so $ZERO_RUNNER is NOT exported to subsequent shell calls. Instead we announce the
# runner's absolute path in additionalContext (it is deterministic — see SHIM_PATH below)
# and the bundled 'zero' skill tells the agent to invoke the runner by that path.
#
...
# session into an extension-owned runtime home and point a one-line shim at the installed
# entry, so the runner path stays a single, stable path and per-call overhead is just Node
# startup (~200ms) — not an npx resolve (~1s). The same code runs on Windows, macOS,
# Linux, and cloud sandboxes; only how we obtain Node varies:
...
#     CLI, and the shim. Extension-owned and disposable — safe to delete and rebuild. Kept
#     in its own subdir (not loose in ~/.zero) and contained via explicit --prefix /
#     npm_config_cache paths (NOT via $HOME). Shared across host plugins/extensions.
...
#     is shared across every Zero install method on the machine (extension, skill.md, raw
#     CLI), not just extension agents. Writes are additive (a session merges in alongside
...
# Extension-owned RUNTIME area (Node, npm cache, installed CLI, shim), namespaced under
# the user's ~/.zero so there's a single Zero home. This is NOT the auth/config file: the
# shim leaves $HOME alone, so the login in ~/.zero/config.json is shared with the
# standalone CLI and skill.md-only agents. Override the location with ZERO_PLUGINS_HOME.
...
# Gemini CLI reads hookSpecificOutput.additionalContext (no hookEventName needed) and
# injects it as the first turn in history (interactive) / prepends it to the prompt
# (non-interactive) — the same effect as Claude's SessionStart additionalContext.
emit() {
  local ctx="$1"
  ctx="${ctx//\\/\\\\}"      # backslashes
...
if [ -z "$NODE_BIN" ]; then
  ...
  emit "Zero runner unavailable: no Node runtime and none could be downloaded (or no network egress). Tell the user Zero isn't available in this environment rather than improvising."
  exit 0
fi
...
if [ ! -f "$CLI_ENTRY" ]; then
  emit "Zero runner unavailable: $CLI_PKG@$VERSION could not be installed (likely no npm-registry egress on first run). Tell the user Zero isn't available here rather than improvising."
  exit 0
fi
...
cat >"$SHIM_PATH" <<SHIM
#!/usr/bin/env sh
# Zero runner shim (generated by the zero extension's SessionStart hook).
...
emit "Zero runner ready at $SHIM_PATH — a drop-in for the zero CLI ($CLI_PKG@$INSTALLED_VERSION). Gemini does not export this path to your shell, so invoke the runner by this absolute path (e.g. $SHIM_PATH search \"...\"). Use it for the whole Zero loop (search/get/fetch/review), and follow the bundled 'zero' skill for the workflow and authentication — don't improvise auth or create a wallet."
exit 0
